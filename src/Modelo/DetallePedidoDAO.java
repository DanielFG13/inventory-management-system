...
public class DetallePedidoDAO {
    ...
    public int generar(List<DetallePedido> detallePedidos, int id_pedido) {
        for (int i = 0; i < detallePedidos.size(); i++) {
            ...
            int cantidadNueva = cantidadNuevaExistente(detallePedido);
            if(cantidadNueva >= 0) {
                try {
                    con = conectar.getConnection();
    ...
    public int cantidadNuevaExistente(DetallePedido detallePedido) {
        StockDAO stockDAO = new StockDAO();
        int cantidadActual = stockDAO.cantidadActual(detallePedido.getId_producto());
        return cantidadActual - detallePedido.getCantidad();      
    }
    
    public int cantidadNueva(int cantidadActual, int cantidadPedido) {
        return cantidadActual - cantidadPedido;
    }
